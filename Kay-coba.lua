-- [[ KAY HUB PRO V9.2 - AUTO BYPASS REJOIN VIA HUB ]] --
...
task.spawn(function()
    pcall(function()
        if getgenv then
            -- BACA DARI REJOIN TELEPORT QUEUE
            if getgenv().KayHub_AutoVerified then
                getgenv().KayHub_Verified = true
            end

            -- AUTO TP KE POSISI SEMULA JIKA ADA
...
-- FUNGSI UNTUK TITIP PESAN TELEPORT (BYPASS AUTO REJOIN)
...
RejoinTPBtn.MouseButton1Click:Connect(function()
    ...
    local currentPos = hrp and hrp.Position or nil
    
    setRejoinBypassQueue(currentPos)
    pcall(function()
        if #Players:GetPlayers() <= 1 then
            TeleportService:Teleport(game.PlaceId, LocalPlayer)
        else
...
-- SYSTEM CHECK: JIKA REJOIN VIA HUB -> LANGSUNG UNLOCK SCRIPT
pcall(function()
